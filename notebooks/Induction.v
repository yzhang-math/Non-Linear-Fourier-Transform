...
(** **** Exercise: 2 stars, standard, especially useful (add_assoc)
...
Proof.
  (* FILL IN HERE *)
  intros n m.
  assert (H1: forall nn k: nat, nn*(S k) = nn + nn*k).
  {intros nn k.
  induction nn as [|nn' IHnn'].
  - simpl. reflexivity.
  - simpl. rewrite IHnn'. rewrite add_shuffle3. reflexivity.
  }
  induction n as [|n' IHn'].
  - simpl. induction m as [|m' IHm']. reflexivity. simpl. rewrite <- IHm'. reflexivity.
  - simpl. rewrite H1. rewrite IHn'. reflexivity.
Qed.
(** [] *)
